...
public class CardService {
	...
	public String cardBlock(Long accountNumber, Long cardNumber) {
	...
	public String createCard(Long accountNumber, CardDto cardDto) {
		 Account account = accountRepository.findByAccountNumber(accountNumber).get();
	...
	public void updateCard(Card card, Long cardNumber) {
		 Optional<Card> optionalCard = cardRepository.findByCardNumber(cardNumber);
	        ...
	        switch(fetchedCard.getCardType()){
	            case DEBIT_GLOBAL -> {
	            if(cardLimit != 0 && card.getDailyLimit()<= 50000) 
	            	fetchedCard.setDailyLimit(card.getDailyLimit());
	            }
	            case DEBIT_CLASSIC -> {
	            if(cardLimit != 0 && card.getDailyLimit()<= 40000)
	            	fetchedCard.setDailyLimit(card.getDailyLimit());
	            }
	            case CREDIT_MASTER -> {
	            if(cardLimit != 0 && card.getDailyLimit()<= 100000)
	            	fetchedCard.setDailyLimit(card.getDailyLimit());
	            }
	            case CREDIT_PREMIUM -> {
	            if(cardLimit != 0 && card.getDailyLimit()<= 75000)
	            	fetchedCard.setDailyLimit(card.getDailyLimit());
	            }
	        }
	        if(card.getPin()!=null) fetchedCard.setPin(card.getPin());
